...
def process_single_syllogism(inputs, varnames):
    ...
    for syllogism in inputs:
        ...
        if syllogism[0] == 1:
            ...
            print(11111)
            finalq = 'for bitmask in range(2**'+str(len(varnames)-2)+'):\n    l = []\n    j = 0\n    t = bitmask\n    for v in range('+str(len(varnames)-2)+'):\n        if t%2 == 1:\n            l.append(1)\n        else:\n            l.append(0)\n        t//=2\n        j += 1\n    q = genQuery(varnames, '+str(syllogism)+', 1, 1, l)\n    if eval(q) != -1:\n        exec(q+" += '+str(2**existances)+'")'
            print(22222)
            print(finalq)
            exec(finalq) 
            #ravelq = eval(q+".ravel()")
...
def eval_single_syllogism(inputs, res_syllogism):
    ...
    try: 
        solve, errors, existances = process_single_syllogism(inputs, varnames)
    except:
        return False 
...
Choice: """) 
        ...
        if n==1:
            showinput = False 
            stdout.write("Show input syllogisms? (y/n): ")
            temp = stdin.readline()
            if temp[0].lower() == 'y':
                showinput = True

            simpleout = False 
            stdout.write("Show simplified output? (y/n): ")
            temp = stdin.readline()
            if temp[0].lower() == 'y':
                simpleout = True
            
            #stdout.write("That's work in progress. \n\n")
            stdout.write("Enter filename (.syl or .txt): ") 
            ...
            #show inputs
            if showinput:
                stdout.write("INPUT SYLLOGISMS: \n") 
                for syllogism in inputs:
                    stdout.write(caseGenerator.printSyllogism(syllogism))
                    stdout.write("\n")
                stdout.write("\n")

            if len(res_syllogisms) == 0:
                deftrue = []
                deffalse = []
                dontknow = []
                ...
                for res_syllogism in res_syllogisms:
                    res, antires = eval_res(solve, varnames, res_syllogism, errors, existances)
                    if simpleout:
                        if res:
                            outfile.write("1\n")
                    ...
                    stdout.write("EVALUATING OUTPUT SYLLOGISM: ")
                    stdout.write(caseGenerator.printSyllogism(res_syllogism))
                    stdout.write("\n")
                    if res:
                        stdout.write("This syllogism is definitely true. \n")
                    elif antires:
